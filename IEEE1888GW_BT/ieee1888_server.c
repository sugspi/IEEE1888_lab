...
int ieee1888_server_print_wsdl(char* buffer, int socket, const char* path){
  ...
  len=sizeof(sa);
  if(getsockname(socket, (struct sockaddr*)&sa, (socklen_t*)&len)==0){ // modified by Hiroyuki Ikegami 2013-09
     char buf[100];
...
void* ieee1888_server_http_proc(void* args){
  ...
  write(cs,"\r\n\r\n",4); // added by Motomasa Tanaka (motomasa-tanaka@mayekawa.co.jp) 2013-10-06
  close(cs);
...
int ieee1888_server_create(int port){
  ...
  for(res=res0; res && smax < FD_SETSIZE; res=res->ai_next){
    ...
    if(bind(s[smax],(struct sockaddr *)res->ai_addr, res->ai_addrlen)<0){
      ...
      //fprintf(stderr,"bind(2) failed: [%s]:%s\n",sopt.host,sopt.service); // modified by Hiroyuki Ikegami 2013-09
      close(s[smax]);
      s[smax]=-1;
      continue;
    }else{
